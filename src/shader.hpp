...
public:
  ...
  unsigned int link_shader_program(unsigned int vertex, unsigned int fragment);

  void use(unsigned int id);

  shader(std::vector<std::string> vertex_paths, std::vector<std::string> fragment_paths);
