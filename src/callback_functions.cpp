...
variable_wrapper::variable_wrapper()
    : projection_mat(glm::mat4(1.0f)), model_mat(glm::mat4(1.0f)),
      view_mat(glm::mat4(1.0f)), window_height(500), window_width(700),
      fractal(0)
{
}
...
//{
//     glm::vec3 mouse((float)mouse_xpos, (float)window_height -
//     (float)mouse_ypos, 100.0f);
//
//     // fails when matrix is non-invertible, determinant is zero
//     glm::vec3 world_proj = glm::unProject(mouse, view_mat, projection_mat,
//     glm::vec4(0.0f, 0.0f, (float)window_width, (float)window_height));
//
//     glm::vec3 eye = glm::vec3(glm::inverse(view_mat)[3]);
//
//     glm::vec3 ray = glm::normalize(world_proj - eye);
//
//     glm::mat4 inv_view = glm::translate(glm::mat4(1.0f), ray*delta) *
//     glm::inverse(view_mat);
//
...
void key_callback(GLFWwindow* main_window, int key, int scancode, int action,
                  int mods)
{
    void* data = glfwGetWindowUserPointer(main_window);
    variable_wrapper* wrapper = reinterpret_cast<variable_wrapper*>(data);
